...
#---------------------------------------------------------------------------------------------
#---------------------------------------------------------------------------------------------
class   CrawlerSoWendaSub(CrawlerSiteScrollSub):
    ...
    def __init__(self,crawlerType,queueDict,configParaInfo,index):
        ...
        #log
        logpath = "so_wenda/sub/" + str(index) 
    ...
    async def  dealSubCrawlerAsFloor(self,page,crawlerInfo,index):
        ...
        self.printLogInfo("CrawlerSoWendaSub->dealSubCrawlerAsFloor() : enter")

        ret,replyInfoBuf = await self.dealCompoundCrawlerPage(page,crawlerInfo,index)

        self.printLogInfo("CrawlerSoWendaSub->dealSubCrawlerAsFloor() : exit ret={}".format(ret))
    ...
    async def  dealSubCrawlerAsDefUrl(self,page,crawlerInfo,index):
        ...
        self.printLogInfo("CrawlerSoWendaSub->dealSubCrawlerAsDefUrl() : enter")
        
        ret,replyInfoBuf = await self.dealDefUrlCrawlerPage(page,crawlerInfo,index)

        self.printLogInfo("CrawlerSoWendaSub->dealSubCrawlerAsDefUrl() : exit ret={}".format(ret))
    ...
    async def dealDefUrlCrawlerPage(self,page,crawlerInfo,index):
        self.printLogInfo("CrawlerSoWendaSub->dealDefUrlCrawlerPage() : enter")
        
        replyBuf = []
        ret,replyBuf = await self.grapAllPageData(page,crawlerInfo,index)
        
        self.printLogInfo("CrawlerSoWendaSub->dealDefUrlCrawlerPage() : exit")
        return ret,replyBuf
    ...
    async def  dealCompoundCrawlerPage(self,page,crawlerInfo,index):
        ...
        self.printLogInfo("CrawlerSoWendaSub->dealCompoundCrawlerPage() : enter")

        replyBuf = []
        ret,replyBuf = await self.grapAllPageData(page,crawlerInfo,index)
        
        self.printLogInfo("CrawlerSoWendaSub->dealCompoundCrawlerPage() : exit")
    ...
    async def grapAllPageData(self,page,crawlerInfo,index):
        ...
        self.printLogInfo("CrawlerSoWendaSub->grapAllPageData() : enter")
        ...
        #抓取满意答案
        ret,replyInfo = await self.grapBestReplyInfo(page,crawlerInfo)
        if ret == RetVal.Suc.value:
            replyBuf.append(replyInfo)
            self.printReplyInfo(replyInfo)
        else:
        ...
        self.printLogInfo("CrawlerSoWendaSub->grapAllPageData() : exit")

        return ret,replyBuf

     #----------------------------------------------------------------------------------------
    #---抓取满意答案：这条回复会一直出现在头部，无论是不是翻页
    async def grapBestReplyInfo(self,page,crawlerInfo):
        
        self.printLogInfo("CrawlerSoWendaSub->grapMainInfo() : enter")
        ...
        #----context
        #--文本
        selector = ".mod-resolved-ans .bd .resolved-cnt"
        ret,curText = await self.pyteerEx.getElementContext(page,selector)
        replyInfo.text = CommonTool.strStripRet(curText)      

        self.printLogInfo("CrawlerSoWendaSub->grapMainInfo() : exit")
        if len(replyInfo.publisher)>0 and len(replyInfo.text)>0:
            ret = RetVal.Suc.value 
    ...
    async def grapAllReplyInfo(self,page,crawlerInfo,strTimeYear):
        ...
        self.printLogInfo("CrawlerSoWendaSub->grapAllReplyInfo() : enter")
        ...
        #-----main info
        selector          = ".mod-other-ans .answers .bd ul li"# 除满意回复外的其他所有回复
        ...
        while True:
            ...
            if await self.pyteerEx.findElement(page,nextPageTagCss)==RetVal.Suc.value:
                ...
                break
            #判断是否成功翻到下一页
            currPageIndexTagCss=".pagination .pages b"
            while True:
    ...
    #     self.printLogInfo("CrawlerSoWendaSub->grapSubReplyInfo() : enter")  
    ...
    #     self.printLogInfo("CrawlerSoWendaSub->grapSubReplyInfo() : exit")    
        
    #     return ret,replyBuf
    # #----------------------------------------------------------------------------------------
    # #---抓取主回复的子回复
    # async def grapItemSubReplyInfo(self,page,element,crawlerInfo,strTimeYear):
    #     self.printLogInfo("CrawlerSoWendaSub->grapMainReplyInfo() : enter")
    ...
    #     self.printLogInfo("CrawlerSoWendaSub->grapMainReplyInfo() : exit")
    ...
    async def clickButtonMore(self,page):
        ...
        self.printLogInfo("CrawlerSoWendaSub->clickButtonMore() : enter")
        ...
        while count > 0:
            ...
            if ret == RetVal.Suc.value:
                self.printLogInfo("CrawlerSoWendaSub->clickButtonMore() : find more button")
                continue
            else:
                break
        ...
        self.printLogInfo("CrawlerSoWendaSub->clickButtonMore() : exit")
    ...
    async def grapMainInfo(self,page,crawlerInfo):
        ...
        #---publisher
        selector    = ".mod-q .ask-author"
        ret,curText =  await self.pyteerEx.getElementContext(page,selector)
        replyInfo.publisher = CommonTool.strStripRet(curText)
        
        #---time
        selector    =  ".mod-q .text>span:nth-last-child(1)"
        ret,curText =  await self.pyteerEx.getElementContext(page,selector)
        if len(curText) > 0:
            curText = curText.replace(".","-")
            curText = curText + " 00:00:00"
        ...
        #--reads
        selector    =  ".mod-q .text>span:nth-last-child(2)"
        ret,curText =  await self.pyteerEx.getElementContext(page,selector)
        curText = curText.split("被浏览",1)[1].split("次",1)[0]
        if len(curText) > 0:
            replyInfo.reads   =   CommonTool.strToInt(curText)       
        #---title
        selector    = ".mod-q .title"
        ret,curText =  await self.pyteerEx.getElementContext(page,selector)
    ...
    async def dealMainScrollBefor(self,page,crawlerInfo):
        ...
        self.printLogInfo("CrawlerSoWendaSub->dealMainScrollBefor() : enter")
        selector  = ".mod-see-other"#查看其它标签
        ret = await self.pyteerEx.clickHoverElement(page,selector,1,1)
        self.printLogInfo("CrawlerSoWendaSub->dealMainScrollBefor() : exit")
        return ret
    #--------------------------------------------------------------------------------------
    async def  waitforPageLoad(self,page):
        self.printLogInfo("CrawlerSoWendaSub->waitforPageLoad() : enter")
        contentTagCss = "#js-detail"
        ret = await self.pyteerEx.waitPage(page,contentTagCss,4)
        if ret == RetVal.Suc.value:
            wenda_url = "https://wenda.so.com/"
            if wenda_url.find(page.target.url) != -1:
                ret = RetVal.Fail.value
        self.printLogInfo("CrawlerSoWendaSub->waitforPageLoad() : exit")
        return ret
