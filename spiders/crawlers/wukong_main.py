...
from  crawlers.wukong_sub        import *

#---------------------------------------------------------------------------------------------    
#---------------------------------------------------------------------------------------------
#----------------------------------------------------------------------------------------------
#----------------------------------------------------------------------------------------------
class CrawlerWukong(CrawlerBase):
    ...
    def crawlerScroll(self,loop,crawlerTaskInfo,configParaInfo):
        print("CrawlerWukong->crawlerScroll() :enter")

        if crawlerTaskInfo.grap_type   ==  GrapTypeEm.Compoundword:

            print("CrawlerWukong->crawlerScroll() :GrapTypeEm.Compoundword")
            ...
            print("CrawlerWukong->crawlerScroll() :GrapTypeEm.Url")
            ...
            print("CrawlerWukong->crawlerScroll() :GrapTypeEm.User")
            self.crawlerDefUser(loop,crawlerTaskInfo,configParaInfo )

        print("CrawlerWukong->crawlerScroll() :leave")
        
    #------------------------------------------------------------------------------------
    #---compoundword
    def crawlerDefCompoundword(self,loop,crawlerTaskInfo,configParaInfo):

        print("CrawlerWukong->crawlerDefCompoundword():enter ")
        ...
        #----main
        objMain = CrawlerWukongMain(crawlerTaskInfo.grap_type,queueDict,configParaInfo)
        ...
        for i in range(1,subColNum):
            objSub  = CrawlerWukongSub(crawlerTaskInfo.grap_type,queueDict,configParaInfo,i)
        ...
        print("CrawlerWukong->crawlerDefCompoundword():leave ")
    ...
    def crawlerDefUrl(self,loop,crawlerTaskInfo,configParaInfo):
        ...
        print("CrawlerWukong->crawlerDefUrl():enter ")
        
        colTastList = []
        ObjSubList  = []
        queueDict   = self.queueDict

        objSub  = CrawlerWukongSub(crawlerTaskInfo.grap_type,queueDict,configParaInfo,1)
        ObjSubList.append(objSub)
        ...
        print("CrawlerWukong->crawlerDefUrl():exit ")
...
#---------------------------------------------------------------------------------------------
#------悟空抓取首页类
class CrawlerWukongMain(CrawlerSiteScrollMain):
    def __init__(self,crawlerType,queueDict,configParaInfo):
        ...
        #--搜索按钮、点击搜索
        self.cssSearchDict     = {
                                  'inputBox':  "div.search-box>div.widget-search-input>form>div.input-group.clearfix>input",  
                                  'clkButton':  "div.search-box>div.widget-search-input>form>div.input-group.clearfix>div.btn-submit>button",
        ...
        self.cssMainItemHrefText = {
                                    'main_sub':"div.index-question-list >div.w-feed.search-all >div.w-feed-container >div.question-v3",
                                    'main_sub_item':"div.question-title >h2 >a",
                                   }
        ...
        #log
        self.log_inits("wukong/main",2,2,logging.INFO)
    
    #--------------------------------------------------------------------------------------
    #--------------------------------------------------------------------------------------
    #search compoundword
    async def searchWord(self,page,compoundword):
        self.printLogInfo("CrawlerWukongMain->searchWord() : enter")
        return await self.searchWordBase(page,compoundword)

    #--------------------------------------------------------------------------------------
    #----waitforPageLoad 
    async def waitforPageLoad(self,page):
        self.printLogInfo("CrawlerWukongMain->waitforPageLoad() : enter")
    ...
    async def dealSubCrawlerAsItem(self,page,crawlerInfo,index):
        ...
        self.printLogInfo("CrawlerWukongMain->dealSubCrawlerAsItem() : enter")
        ...
        self.printLogInfo("CrawlerWukongMain->dealSubCrawlerAsItem() : leave curItemNo={} grap_count={}".format(nItemNo,crawlerInfo.vscroll_grap_count))    
        return RetVal.Suc.value,grapTopBuf
    
    """
    #---------------------------------------------------------------------------------------------
    # override func scrollAuto load page completely
    async def autoScrollAsVScroll(self,page,crawlerInfo):
    #async def scrollAuto(self,page,step):
        step = self.autoScrollStep
        ret1 = RetVal.Suc.value
        self.printLogInfo("CrawlerWukongMain->scrollAuto(): enter")
        while True:
            ret = await self.pyteerEx.scrollAutoCur(page,step)
            if ret != RetVal.Suc.value:
                self.printLogInfo("CrawlerWukongMain->scrollAuto(): scroll fail => error")
                continue
            else:
                ret,curText = await self.pyteerEx.getElementContext(page,self.cssGetElement['loadMoreContent'])
                if ret != RetVal.Suc.value:
                    self.printLogInfo("CrawlerWukongMain->scrollAuto(): scroll fail => error1")
                else:
                    if curText == self.loadMoreContentText:
                        break
                    else:
                        continue
        self.printLogInfo("CrawlerWukongMain->scrollAuto(): leave")
        return ret1
    """
    #-------------------------------------------------------------------------------------------
